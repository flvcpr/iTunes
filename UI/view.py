...
class View(ft.UserControl):
    ...
    def load_interface(self):
        ...
        self._ddAlbum = ft.Dropdown(label="Album",
                                    on_change=self._controller.getSelectedAlbum)
        self._btnAnalisiComp = ft.ElevatedButton(text="Analisi Componente",
                                               on_click=self._controller.handleAnalisiComp)
        row2 = ft.Row([ft.Container(self._ddAlbum, width=300), ft.Container(self._btnAnalisiComp, width=300)],
                      alignment=ft.MainAxisAlignment.CENTER)
        self._page.controls.append(row2)

        #row3
        self._txtInSoglia = ft.TextField(label="Soglia")
        self._btnSetAlbum = ft.ElevatedButton(text="Crea Grafo",
